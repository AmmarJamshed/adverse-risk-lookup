...
class RegulatorySource(Base, TimestampMixin):
    ...
    jurisdiction: Mapped[Optional[str]] = mapped_column(String(50), index=True)
...
class HorizonItem(Base, TimestampMixin):
    __tablename__ = "horizon_items"

    id: Mapped[uuid.UUID] = mapped_column(Uuid, primary_key=True, default=_uuid)
    title: Mapped[str] = mapped_column(String(1000), nullable=False)
    jurisdiction: Mapped[str] = mapped_column(String(50), index=True)
    regulator: Mapped[str] = mapped_column(String(100), index=True)
    ...
    source_id: Mapped[Optional[uuid.UUID]] = mapped_column(Uuid, ForeignKey("regulatory_sources.id"))
    source_name: Mapped[Optional[str]] = mapped_column(String(255))
    status: Mapped[str] = mapped_column(String(50), default="pending_assessment", index=True)
    priority: Mapped[str] = mapped_column(String(20), default="medium")
    tags: Mapped[list] = mapped_column(JSONType, default=list)
    candidates: Mapped[list] = mapped_column(JSONType, default=list)


class PolicyDoc(Base, TimestampMixin):
...
class ControlItem(Base, TimestampMixin):
    __tablename__ = "control_items"

    id: Mapped[uuid.UUID] = mapped_column(Uuid, primary_key=True, default=_uuid)
    code: Mapped[str] = mapped_column(String(50), unique=True, nullable=False)
    title: Mapped[str] = mapped_column(String(500), nullable=False)
    policy_id: Mapped[Optional[uuid.UUID]] = mapped_column(Uuid, ForeignKey("policy_docs.id"))
    owner: Mapped[Optional[str]] = mapped_column(String(255))
    type: Mapped[str] = mapped_column(String(50), default="detective")
    status: Mapped[str] = mapped_column(String(50), default="operating")
...
class Obligation(Base, TimestampMixin):
    ...
    source_horizon_id: Mapped[Optional[uuid.UUID]] = mapped_column(Uuid, ForeignKey("horizon_items.id"))
...
class GapCase(Base, TimestampMixin):
    ...
    obligation_id: Mapped[uuid.UUID] = mapped_column(
        Uuid, ForeignKey("obligations.id"), nullable=False, index=True
    )
    title: Mapped[str] = mapped_column(String(500), nullable=False)
    status: Mapped[str] = mapped_column(String(50), default="open", index=True)
    gap_status: Mapped[str] = mapped_column(String(50), default="gap", index=True)
    owner: Mapped[Optional[str]] = mapped_column(String(255))
    jurisdiction: Mapped[Optional[str]] = mapped_column(String(50))
    remediation_notes: Mapped[Optional[str]] = mapped_column(Text)
    mappings: Mapped[list] = mapped_column(JSONType, default=list)
